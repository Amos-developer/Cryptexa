    ...
    public function store(Request $request)
    {
        ...
        if (in_array($network, ['usdtbsc', 'usdcbsc', 'bnbbsc'])) {
            ...
            if (!preg_match('/^0x[a-fA-F0-9]{40}$/', $address)) {
                return back()->withErrors(['address' => 'Invalid BEP20/BSC address format. Must start with 0x and be 42 characters.'])->withInput();
            }
        } elseif ($network === 'usdttrc20') {
            // TRC20 addresses start with T and are 34 characters
            if (!preg_match('/^T[a-zA-Z0-9]{33}$/', $address)) {
                return back()->withErrors(['address' => 'Invalid TRC20 address format. Must start with T and be 34 characters.'])->withInput();
            }
        }
        ...
        if ($user->withdrawal_address === $address && $user->withdrawal_network === $network) {
            return back()->withErrors(['address' => 'New address must be different from current address.'])->withInput();
        }
        ...
        return redirect()->route('account.settings')->with('success', 'Withdrawal address saved successfully!');
    }
}
