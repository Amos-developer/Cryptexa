...
@section('page-title', 'Edit Withdrawal')
@section('page-description', 'Modify withdrawal details')
...
<div class="container-fluid" style="padding:20px">
  <div class="edit-card">
    <div class="card-header">
      ...
      <h2 class="card-title">Edit Withdrawal</h2>
      <p class="card-subtitle">Update withdrawal #{{ $withdrawal->id }}</p>
    </div>
    ...
    <form action="{{ route('admin.withdrawals.update', $withdrawal) }}" method="POST">
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <select name="user_id" class="form-select @error('user_id') error @enderror" required>
            ...
            @foreach($users as $user)
              <option value="{{ $user->id }}" {{ old('user_id', $withdrawal->user_id) == $user->id ? 'selected' : '' }}>{{ $user->username }} ({{ $user->email }})</option>
            @endforeach
          </select>
          @error('user_id')
        ...
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <input type="number" step="0.01" min="0.01" name="amount" class="form-input @error('amount') error @enderror" value="{{ old('amount', $withdrawal->amount) }}" required>
          </div>
          @error('amount')
            <span class="error-message">{{ $message }}</span>
          @enderror
        ...
        <div class="form-group">
          ...
          <select name="currency" class="form-select @error('currency') error @enderror" required>
            <option value="usd" {{ old('currency', $withdrawal->currency) == 'usd' ? 'selected' : '' }}>USD</option>
            <option value="btc" {{ old('currency', $withdrawal->currency) == 'btc' ? 'selected' : '' }}>BTC</option>
            <option value="eth" {{ old('currency', $withdrawal->currency) == 'eth' ? 'selected' : '' }}>ETH</option>
            <option value="usdt" {{ old('currency', $withdrawal->currency) == 'usdt' ? 'selected' : '' }}>USDT</option>
          </select>
          @error('currency')
            <span class="error-message">{{ $message }}</span>
          @enderror
        ...
        <div class="form-group">
          ...
          <textarea name="address" class="form-textarea @error('address') error @enderror" required>{{ old('address', $withdrawal->address) }}</textarea>
          @error('address')
            <span class="error-message">{{ $message }}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <input type="text" name="txid" class="form-input @error('txid') error @enderror" value="{{ old('txid', $withdrawal->txid) }}" placeholder="Optional">
          @error('txid')
            <span class="error-message">{{ $message }}</span>
          @enderror
        </div>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="btn btn-submit">✔ Update Withdrawal</button>
      </div>
    </form>
  </div>
</div>
