...
@section('page-title', 'Create Manual Deposit')
@section('page-description', 'Add a manual deposit for a user')
...
.card-icon{width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#11998e,#38ef7d);display:inline-flex;align-items:center;justify-content:center;font-size:36px;margin-bottom:16px;box-shadow:0 8px 24px rgba(17,153,142,.3)}
...
.form-input:focus,.form-select:focus{outline:0;border-color:#11998e;box-shadow:0 0 0 3px rgba(17,153,142,.1)}
...
.btn-submit{background:linear-gradient(135deg,#11998e,#38ef7d);color:#fff}
.btn-submit:hover{transform:translateY(-2px);box-shadow:0 6px 20px rgba(17,153,142,.4)}
...
<div class="container-fluid" style="padding:20px">
  <div class="edit-card">
    <div class="card-header">
      ...
      <h2 class="card-title">Create Manual Deposit</h2>
      <p class="card-subtitle">Add a deposit transaction manually</p>
    </div>
    ...
    <form action="{{ route('admin.deposits.store') }}" method="POST">
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-select @error('status') error @enderror" required>
            ...
            <option value="confirming" {{ old('status') == 'confirming' ? 'selected' : '' }}>Confirming</option>
        ...
        <div class="form-group">
          ...
          <input type="text" name="payment_id" class="form-input @error('payment_id') error @enderror" value="{{ old('payment_id') }}" placeholder="Optional">
          @error('payment_id')
            <span class="error-message">{{ $message }}</span>
          @enderror
        </div>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="btn btn-submit">✔ Create Deposit</button>
      </div>
    </form>
  </div>
</div>
