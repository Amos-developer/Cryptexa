...
@section('page-description', 'Modify deposit details')
...
<div class="container-fluid" style="padding:20px">
  <div class="edit-card">
    <div class="card-header">
      ...
      <h2 class="card-title">Edit Deposit</h2>
      <p class="card-subtitle">Update deposit #{{ $deposit->id }}</p>
    </div>
    ...
    <form action="{{ route('admin.deposits.update', $deposit) }}" method="POST">
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <select name="user_id" class="form-select @error('user_id') error @enderror" required>
            ...
            @foreach($users as $user)
              <option value="{{ $user->id }}" {{ old('user_id', $deposit->user_id) == $user->id ? 'selected' : '' }}>{{ $user->username }} ({{ $user->email }})</option>
            @endforeach
          </select>
          @error('user_id')
        ...
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <input type="number" step="0.01" min="0.01" name="amount" class="form-input @error('amount') error @enderror" value="{{ old('amount', $deposit->amount) }}" required>
          </div>
          @error('amount')
            <span class="error-message">{{ $message }}</span>
          @enderror
        ...
        <div class="form-group">
          ...
          <select name="currency" class="form-select @error('currency') error @enderror" required>
            ...
            <option value="btc" {{ old('currency', $deposit->currency) == 'btc' ? 'selected' : '' }}>BTC</option>
            <option value="eth" {{ old('currency', $deposit->currency) == 'eth' ? 'selected' : '' }}>ETH</option>
            <option value="usdt" {{ old('currency', $deposit->currency) == 'usdt' ? 'selected' : '' }}>USDT</option>
          </select>
          @error('currency')
            <span class="error-message">{{ $message }}</span>
          @enderror
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-select @error('status') error @enderror" required>
            ...
            <option value="waiting" {{ old('status', $deposit->status) == 'waiting' ? 'selected' : '' }}>Waiting</option>
            <option value="confirming" {{ old('status', $deposit->status) == 'confirming' ? 'selected' : '' }}>Confirming</option>
        ...
        <div class="form-group">
          ...
          <input type="text" name="payment_id" class="form-input @error('payment_id') error @enderror" value="{{ old('payment_id', $deposit->payment_id) }}" placeholder="Optional">
          @error('payment_id')
            <span class="error-message">{{ $message }}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <input type="text" name="txid" class="form-input @error('txid') error @enderror" value="{{ old('txid', $deposit->txid) }}" placeholder="Optional">
          @error('txid')
            <span class="error-message">{{ $message }}</span>
          @enderror
        </div>
      ...
      <div class="form-actions">
        <a href="{{ route('admin.deposits.index') }}" class="btn btn-cancel">← Cancel</a>
        <button type="submit" class="btn btn-submit">✔ Update Deposit</button>
      </div>
    </form>
  </div>
</div>
